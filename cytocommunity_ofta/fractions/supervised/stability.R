...
ThisStep_OutputFolderName <- paste0("/home/owkin/project/cytocommunity_results/fractions/supervised/experiments/", timestamp, "/ensemble")

if (file.exists(ThisStep_OutputFolderName)){
    unlink(ThisStep_OutputFolderName, recursive=TRUE)} 
...
dirs <- list.dirs(paste0("/home/owkin/project/cytocommunity_results/fractions/supervised/experiments/", timestamp, "/crossval"), recursive = TRUE)
...
for (p in 1:length(Fold_FolderName)) {
  ...
  for (q in 1:nrow(ImageNameList)) {
    ...
    TimeName <- ImpStr[[1]][length(ImpStr[[1]])-1]
    FoldName <- ImpStr[[1]][length(ImpStr[[1]])]
...
Sys.time()
rm(list = setdiff(ls(), "timestamp"))
library(diceR)
ThisStep_OutputFolderName <- paste0("/home/owkin/project/cytocommunity_results/fractions/supervised/experiments/", timestamp, "/ensemble/")
print(ThisStep_OutputFolderName)
...
for (kk in 1:length(Image_dirs)) {
  ...
  for (i in 1:length(allSoftClustFile)) {
    ...
    finalClass_FileName <- paste(c(Image_dirs[kk], "/TCNLabel_MajorityVoting_", i, ".csv"), collapse = '')
